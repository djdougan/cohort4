...
test("Test character removal", () => {
  expect(syntax.removeChosenOne("Full stack web developer", "ae")).toMatch(
    "Full stck wb dvlopr"
  );

  expect(
    syntax.removeChosenOne("Supercalifragilisticexpialidocious", "aeiouy")
  ).toMatch("Sprclfrglstcxpldcs");
  ...
  expect(
    syntax.removeChosenOne(
      "The quick brown fox jumps over the lazy dog",
      "aeiouy"
    )
  ).toMatch("Th qck brwn fx jmps vr th lz dg");
  ...
  ).not.toMatch("Th qck brwn fx jmps vr th lzy dg");
});
...
test("Test update element in array", () => {
  ...
  expect(syntax.updateArray(["Bob", "Sam"], "Bob", "Tom")).toEqual([
    "Tom",
    "Sam",
  ]);
  expect(syntax.updateArray(["Bob", "Sam"], "Sam", "Tom")).toEqual([
    "Bob",
    "Tom",
  ]);
});
...
test("Test add a value to each element in array", () => {
  ...
  expect(syntax.addToEach([11, 15, 31, 35, 56], 5)).toEqual([
    16,
    20,
    36,
    40,
...
test("Test toUpperCaseArray ", () => {
  expect(syntax.toUpperCaseArray(["Sam", "Sara", "Bob"])).toEqual([
    "SAM",
    "SARA",
    "BOB",
  ]);
  expect(syntax.toUpperCaseArray(["luke", "kim", "phil"])).toEqual([
    "LUKE",
    "KIM",
    "PHIL",
  ]);
...
test("Test: keyValueLookUp", () => {
  ...
  expect(syntax.keyValueLookUp(6, employees)).toEqual({
    id: 6,
    name: "Jackie",
  });
